   ...
   class StairsExporter
   {
          ...
          PlacementSetter placementSetter, ProductWrapper productWrapper)
      {
         ...
         foreach (IFCAnyHandle component in components)
         {
            ...
            if (isSubStair)
            {
               string localStairType = IFCAnyHandleUtil.GetEnumerationAttribute(component, ExporterCacheManager.ExportOptionsCache.ExportAs4 ? "PredefinedType" : "ShapeType");
         ...
         for (int ii = 0; ii < numFlights - 1; ii++)
         {
            string stairType = null;
            if (ExporterCacheManager.ExportOptionsCache.ExportAs4)
               stairType = IFCAnyHandleUtil.GetEnumerationAttribute(stairHnd, "PredefinedType");
            else
               stairType = IFCAnyHandleUtil.GetEnumerationAttribute(stairHnd, "ShapeType");
      ...
      public static void Export(ExporterIFC exporterIFC, Element element, GeometryElement geometryElement, ProductWrapper productWrapper)
      {
         ...
         ExporterUtil.GetExportType(exporterIFC, element, out ifcEnumType);
         IFCFile file = exporterIFC.GetFile();
